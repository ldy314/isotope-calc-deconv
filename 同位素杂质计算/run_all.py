...
def write_report(results_by_z, sweep, val_std):
    ...
    A("  M0 法轻→重、基峰法重→轻逐级三角扣除。5 ppm 下基峰(t) 与 M0(t−1) 位置重合，二者量级一致、可互校验；但链端存在固有偏差，并非严格等价，本报告以 **M0·积分** 为准。")
    ...
    A("- 基峰法与 M0 法量级一致，验证了「5 ppm 下基峰(t) 与 M0(t−1) 位置重合」的前提；但链端存在固有偏差，两法并非严格等价。")
    A("")
