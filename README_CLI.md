...
> **第一次接手这个仓库？先读 `AGENTS.md`（操作手册 + 高危坑清单）、`ALGORITHMS.md`（算法总纲）和 `MEMORY.md`（常数与决策台账）。**
...
> `chem.py` 只覆盖"理论谱 / 杂质枚举 / 通用解卷积 / 格式转换"。真正交付报告的另外两条线在子目录里，**都有原生参数化生成器（杂质含量那条）与 xlsm，不要另起炉灶**。两条线的算法见 `ALGORITHMS.md` §3 / §4。
...
```bash
cd "同位素取代率计算"
"$PY" embedded_engine.py --xlsm "输入表.xlsm"   # 读「输入」表 → 结果写 %TEMP%\<基名>_*.csv
"$PY" _selftest.py                              # 离线自测：自动造同布局输入表 → 跑引擎 → 打印结果
```
- 方法：**质心法**（主）+ **包络序号法**（独立交叉验证）。算法细节见 `ALGORITHMS.md` §4；本批结果见 `解卷积/20260914分析/report.md`。
- 输入表单元格布局见 `embedded_engine.py` 文件头（B5/B6 分子式、B7 标记原子数、B8 电荷态、B10 数据目录、B11/D11 RT 窗秒、B13/B14 质量轴 a/c、B15 自动重标定、B16 天然对照、B19:C38 样品表、B40 解释器）。
- ⚠️ **本目录目前没有 `.xlsm`，也没有 `build_xlsm.py`**（Excel 前端尚未创建）。引擎源码已就绪：按上表布局造一张输入表即可直接跑，`_selftest.py` 会自动生成。产出线 C 的 `build_xlsm.py` 只在 `同位素杂质计算/` 下。
...
> `同位素杂质计算.xlsm` 的重建需要 **Excel COM**（本机可用，Office 16.0）；重建会覆盖交付用的 xlsm，务必**先备份**，并核对内嵌 `EngineSrc` 表与「方法说明」页文案是否同步。
